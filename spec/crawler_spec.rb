...
describe JekyllImageData::Crawler do
  ...
  let(:collection_images) { site.collections["collection"].docs[0].data["images"] }
  ...
  context "When collection page contains images in Markdown format" do
    describe "Crawler.crawls" do
      it "crawls png image data" do
        expect(collection_images[0]["url"]).to eq("/media/images/800x600.png")
        expect(collection_images[0]["alt"]).to eq("800x600 png")
      end

      it "crawls jpg image data" do
        expect(collection_images[1]["url"]).to eq("/media/images/800x600.jpg")
        expect(collection_images[1]["alt"]).to eq("800x600 jpg")
      end

      it "crawls gif image data" do
        expect(collection_images[2]["url"]).to eq("/media/images/800x600.gif")
        expect(collection_images[2]["alt"]).to eq("800x600 gif")
      end
  ...
  context "When collection page contains plain HTML images" do
    describe "Crawler.crawl" do
      it "crawls image data" do
        expect(collection_images[4]["url"]).to eq("http://placehold.it/800x600")
        expect(collection_images[4]["alt"]).to eq("800x600 http")
      end

      it "crawls image data if 'alt' attr is not present" do
        expect(collection_images[8]["url"]).to eq("http://placehold.it/800x600")
        expect(collection_images[8]["alt"]).to eq("")
      end
    end
  end

  context "When collection page contains images as liquid files" do
    describe "Crawler.crawl" do
      it "crawls image data" do
        expect(collection_images[5]["url"]).to eq("http://placehold.it/800x600")
        expect(collection_images[5]["alt"]).to eq("800x600 http")
      end

      it "crawls image data if a caption is present" do
        expect(collection_images[6]["url"]).to eq("http://placehold.it/800x600")
        expect(collection_images[6]["alt"]).to eq("800x600 http with caption")
      end
  ...
  context "When collection page is rendered" do
    let(:output) { site.collections["collection"].docs[0].output }
